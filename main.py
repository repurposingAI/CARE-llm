...
def load_cot(
    model_name="/home/habes/NotebookProjects/bboi/sadok/GNN-LLM/data/dalk/llama3/",
    max_gpu_mem="10GB",
    max_cpu_mem="30GB",
):
...
def extract_reranked_triplets(reranked_output):
    ...
    reranked_output = reranked_output.strip()
    ...
    for line in lines:
        ...
        if line.startswith('Reranked Triple'):
            if ':' in line:
            ...
            if ' ——> ' in content:
                ...
                for part in parts:
                    part = part.strip()
                    if '->' in part and 'xxx' not in part.lower():
                ...
                if 'xxx' not in content.lower():
                    triplets.append(content)
    ...
    for triplet in triplets:
        ...
        if (triplet and 
            '->' in triplet and 
            len(triplet) > 3 and 
            'xxx' not in triplet.lower()):
            valid_triplets.append(triplet)
    
    if not valid_triplets:
        for line in lines:
            line = line.strip()
            if '->' in line and 'Reranked Triple' in line:
                if ':' in line:
                    content = line.split(':', 1)[1].strip()
                    content = content.replace(' ——> ', '->')
                    if '->' in content and 'xxx' not in content.lower():
                        valid_triplets.append(content)
...
def prompt_neighbor(neighbor):
    ...
    if isinstance(neighbor, str):
        ...
        if not triplets:
            print("No valid triple found in the input")
       
            return "No relevant neighbor-based evidence found."
        
        neighbor_text = '\n'.join(triplets)
        print(f" Triples extracted for prompt_neighbor:")
        for triplet in triplets:
            print(f"   {triplet}")
    else:
    ...
    try:
        ...
        print(f" Error in prompt_neighbor: {e}")
        return "Error processing neighbor evidence."

        
def prompt_neighbor11111(neighbor):
    template = """
    ...
    formatted_prompt = prompt.format(neighbor=neighbor)
    response_of_KG_neighbor = chat.invoke(formatted_prompt)
    return response_of_KG_neighbor
...
def self_knowledge_retrieval_rerankingok(graph, question):
    template = """
    There is a question and some knowledge graph. The knowledge graphs follow entity->relationship->entity list format.
...
def final_answer(input_text, response_path_evidence, response_of_KG_neighbor):
    ...
    # Fusion des preuves
    all_evidence = response_of_KG_neighbor + "\n\n" + response_path_evidence

    # -----------------------------  
    # PHASE 1 : Chain-of-Thought - Version améliorée
    # -----------------------------
    prompt_cot = f"""
    Below is a medical question and supporting evidence. Think step by step to answer the question.
    ...
    Instructions:
    1. First, identify the drug/disease entities in the question
    2. Examine what the evidence says about these specific entities
    3. Look for direct mentions, but also consider indirect/implied relationships
    4. If evidence mentions related pathways, mechanisms, or similar entities, consider how they might apply
    5. In biomedical contexts, absence of evidence is not evidence of absence
    6. Conclude with whether an interaction exists based on your analysis
    ...
    # -----------------------------  
    # PHASE 2 : Final Answer - Version simplifiée pour Alpaca
         # -----------------------------  
    prompt_final = f"""
    ### System Prompt
    You are a biomedical expert. Answer the question using the provided evidence and reasoning.
    ...
    ### Examples of Correct Format (DO NOT COPY CONTENT)
    Example 1:
    Question: Does Chemical and Drug Induced Liver Injury interact with Proguanil: A.yes B.no
    Answer: Yes. Proguanil has been associated with cases of drug-induced liver injury in clinical reports. Although the reaction is uncommon, hepatotoxicity  is a documented adverse effect of Proguanil, indicating a plausible interaction between the drug and liver injury mechanisms.
     
    Example 2:
    Question: Does Frontometaphyseal dysplasia interact with Metamfetamine: A.yes B.no
    Answer: Yes. Frontometaphyseal dysplasia involves multisystem developmental abnormalities, and patients may receive medications or supportive treatments  that can interact pharmacodynamically or pharmacokinetically with Metamfetamine. While the interaction is not directly documented, the condition's clinical  management and comorbidities create a plausible context for potential interaction.

    Example 3:
    Question: Does Amitriptyline interacts with Marfan Syndrome: A.yes B.no
    Answer: Based on the provided evidence, it is plausible that Amitriptyline interacts with Marfan Syndrome. The drug binds to several receptors and enzymes, including ADRA2B, ADRA2C, CHRM1, and CYP2E1, which are involved in the Marfan Syndrome pathway. However, it is important to note that the provided evidence does not directly document the interaction between Amitriptyline and Marfan Syndrome. Therefore, further research is needed to confirm the interaction. 

    Example 4:
    Question: Does Prazosin interacts with Leukemia-Lymphoma Adult T-Cell: A.yes B.no
    Answer: Based on the provided medical evidence, it is reasonable to conclude that Prazosin interacts with Leukemia-Lymphoma Adult T-Cell. The drug binds to four receptors: ADRA2B, ADRA2C, KCNH7, and ADRA1A, which are involved in the pathogenesis of leukemia and lymphoma. Therefore, it is plausible that Prazosin may interact with Leukemia-Lymphoma Adult T-Cell. 

    ### Important Instructions
    1. Your answer MUST start with either "Yes. " or "No. "
    2. Follow with 1-2 sentences explaining your reasoning
    3. Use natural, fluent medical language
    4. Base your answer on the evidence and reasoning provided
    5. In biomedical contexts, consider plausible mechanisms even if not explicitly stated
    6. DO NOT add any other text before or after your answer
...
def final_prompt(input_text, response_path_evidence, response_of_KG_neighbor):
...
def explore_similar_entities(rdf_graph, target_entities):
    ...
    print(f"\n GRAPH EXPLORATION FOR ENTITIES: {target_entities}")
    ...
    print(f" Graph contains {len(all_entities)} unique entities")
        ...
        for entity in all_entities:
            ...
            if (target_lower in entity_lower or 
                ...
                try:
                    ...
                    print(f" Counting error for '{entity}': {e}")
                    similar_entities[target].append({
                        'entity': entity,
                        'triplet_count': 0,
                        'similarity_score': calculate_similarity(target, entity)
...
def find_shortest_path_in_graph(rdf_graph, entity1, entity2, max_hops=3):
    ...
    try:
        ...
        print(f" {len(results)} SPARQL result(s) found")
...
def find_entity_neighbors(rdf_graph, entity_name, limit=10):
    ...
    try:
        ...
        print(f" Error while searching neighbors: {e}")
...
if __name__ == "__main__":
    rdf_graph = load_txt_to_rdf("/home/habes/NotebookProjects/bboi/sadok/GNN-LLM/data/dalk/datasets/SUBDRKG/SUBDRKG_final.txt")

    #chat = HuggingFacePipeline(pipeline=pipe)
    chatc = load_cot()
    chat = load_llm()

    with open('/home/habes/NotebookProjects/bboi/sadok/GNN-LLM/data/dalk/datasets/SUBDRKG/SUBDRKG_embeddings.pkl', 'rb') as f1:
        entity_embeddings = pickle.load(f1)

    with open('/home/habes/NotebookProjects/bboi/sadok/GNN-LLM/data/dalk/datasets/test/drugbank_embeddings.pkl', 'rb') as f2:
    ...
    for line in lines:
        ...
        try:
            ...
            # 2) Normalize entity embeddings once
            ...
            for kg_entity_pair in question_kg:
                ...
                for kg_entity in kg_entity_pair:
                    # Retrieve embedding index in O(1)
                    kw_index = keyword_to_index[kg_entity]

                    # Normalized embedding of the KG entity
                    kg_emb = keyword_embeddings["embeddings"][kw_index]
                    kg_emb = kg_emb / np.linalg.norm(kg_emb)

                    # FAST cosine similarity: dot product (already normalized)
            ...
            with open("/home/habes/NotebookProjects/bboi/sadok/GNN-LLM/data/dalk/datasets/SUBDRKG/SUBDRKG_final.txt", "r", encoding="utf-8") as f:
            ...
            print(f"\n SIMPLIFIED APPROACH FOR: '{entity1}' ET '{entity2}'")
            ...
            print(f"\n Searching for paths directly…")
            paths = find_shortest_path_in_graph(rdf_graph, entity1, entity2, max_hops=3)
            if paths:
                print(f"\n {len(paths)} Path(s) found")
                for i, path in enumerate(paths):
                    display = f"path {i+1} (length {path['length']}): "
            ...
            for i, p in enumerate(paths, start=1):
                ...
                # Construire la ligne du chemin
                line = f"Chemin {i} (longueur {length}): "
            ...
            # --- Knowledge Graph neighbor-based ---
            #neighbor_new_list = ["->".join(n) for n in neighbors]
            neighbor_new_list = []
            for neighbor_dict in neighbors:
                entity = neighbor_dict['entity']
                # Trouver la clé qui n'est pas 'entity'
            ...
            reasoning = final_prompt(input_text[0], response_path_evidence, response_of_KG_neighbor)
            ...
            print(f" Saved result for the pair : {drug} - {go_term}")

        except Exception as e:
            print(f" Error during processing of the pair : {line.strip()} : {e}")
            continue
